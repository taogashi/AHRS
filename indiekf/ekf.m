...
% mu0,mu1 is state vector,namely attitude quatrenion and bias
% z is measured angle from other sensor.
% w: angle velcoty
% sigma0,sigma1: covariance matrix.
% sigr,sig2:(138) (139),constant value.
% errZ: R (221)
...
q_hat_avg_1 = NormalizeV((exp(1/2*Omega(w_avg)*dt) + 1/48*(Omega(w_hat_1)*Omega(w_hat_0) - Omega(w_hat_0)*Omega(w_hat_1))*dt^2) * q_hat_avg_0);		% (131)
% q_hat_avg_1
% mu1_ = [q_hat_avg_1 ; b1]
...
H = [skew(q_hat_avg_1) zeros(3)];		% NOTE ???
...
S = H*sigma1_*H' + errZ;	%(235)

% 9. Compute the Kalman gain K
K = sigma1_*(H'*inv(S));	%(236)
...
q = multiplyQuaternion(dq_hat_avg_1, q_hat_avg_1);	%(240)
...
sigma1 = (eye(6) - K*H) * sigma1_ * (eye(6) - K*H)' + K*errZ*K'; %(243)
